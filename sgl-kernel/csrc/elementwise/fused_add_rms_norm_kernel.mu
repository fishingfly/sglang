 ...
 */
 ...
 #pragma unroll
      ...
      for (size_t j = n_idx; j < N; j += n_step) {
        ...
        if (j + vlen <= N) {
          ...
          for (int k = 0; k < vlen; k++) {
            if (j + k < N) {
              curr.val_.elem[k] = p_src[j + k];
              res_vec.val_.elem[k] = p_res[j + k];
            }
          }
        }
        #pragma unroll
        ...
        if (j + vlen <= N) {
          *(SrcVec*)(p_res + j) = fused_vec;
          ...
          for (int k = 0; k < vlen; k++) {
            if (j + k < N) {
              p_res[j + k] = fused_vec.val_.elem[k];
            }
          }
        }
      }
  ...
  #pragma unroll
            for (int k = 0; k < vlen; k++) {
              if (j + k < N) {
                fused_vec.val_.elem[k] = p_res[j + k];
                weight_val.val_.elem[k] = weight[j + k];
              }
            }
          }
  #pragma unroll
